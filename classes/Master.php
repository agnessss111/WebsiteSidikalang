...
Class Master extends DBConnection {
	...
	function save_book(){
		...
		if(empty($id)){
			$sql = "INSERT INTO `informasi` set {$data} ";
			$save = $this->conn->query($sql);
			$id = $this->conn->insert_id;
		}else{
			$sql = "UPDATE `informasi` set {$data} where id = '{$id}' ";
			$save = $this->conn->query($sql);
		}
		if($save){
			if(isset($_FILES['img']) && count($_FILES['img']['tmp_name']) > 0){
				if(!is_dir(base_app.'uploads/book_'.$id)){
					mkdir(base_app.'uploads/book_'.$id);
					$data = " `upload_path`= 'uploads/book_{$id}' ";
				}else{
					$data = " `upload_path`= 'uploads/book_{$id}' ";
				}
				$this->conn->query("UPDATE `informasi` set {$data} where id = '{$id}' ");
				foreach($_FILES['img']['tmp_name'] as $k =>$v){
					move_uploaded_file($_FILES['img']['tmp_name'][$k],base_app.'uploads/book_'.$id.'/'.$_FILES['img']['name'][$k]);
				}
			}
	...
	function save_review(){
		...
		if(empty($id)){
			$sql = "INSERT INTO `penghasilan` set {$data} ";
			$save = $this->conn->query($sql);
			$id = $this->conn->insert_id;
		}else{
			$sql = "UPDATE `penghasilan` set {$data} where id = '{$id}' ";
			$save = $this->conn->query($sql);
		}
		if($save){
			if(isset($_FILES['img']) && count($_FILES['img']['tmp_name']) > 0){
				if(!is_dir(base_app.'uploads/review_'.$id)){
					mkdir(base_app.'uploads/review_'.$id);
					$data = " `upload_path`= 'uploads/review_{$id}' ";
				}else{
					$data = " `upload_path`= 'uploads/review_{$id}' ";
				}
				$this->conn->query("UPDATE `penghasilan` set {$data} where id = '{$id}' ");
				foreach($_FILES['img']['tmp_name'] as $k =>$v){
					move_uploaded_file($_FILES['img']['tmp_name'][$k],base_app.'uploads/review_'.$id.'/'.$_FILES['img']['name'][$k]);
				}
			}
	...
	function delete_book(){
		extract($_POST);
		$del = $this->conn->query("DELETE FROM `informasi` where id = '{$id}'");
		if($del){
			$resp['status'] = 'success';
			if(is_dir(base_app.'uploads/book_'.$id)){
				$file = scandir(base_app.'uploads/book_'.$id);
				foreach($file as $img){
					if(in_array($img,array('..','.')))
						continue;
					unlink(base_app.'uploads/book_'.$id.'/'.$img);
				}
				rmdir(base_app.'uploads/book_'.$id);
			}
	...
	function delete_review(){
		extract($_POST);
		$del = $this->conn->query("DELETE FROM `penghasilan` where id = '{$id}'");
		if($del){
			$resp['status'] = 'success';
			if(is_dir(base_app.'uploads/review_'.$id)){
				$file = scandir(base_app.'uploads/review_'.$id);
				foreach($file as $img){
					if(in_array($img,array('..','.')))
						continue;
					unlink(base_app.'uploads/review_'.$id.'/'.$img);
				}
				rmdir(base_app.'uploads/review_'.$id);
			}
			$this->settings->set_flashdata('success',"successfully deleted.");
		}else{
			$resp['status'] = 'failed';
